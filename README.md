...
### 🗂️ Tablespace
A tablespace is a storage location on the file system where the database stores data files (such as tables, indexes, and materialized views).

#### Why Tablespaces Exist
By default, all database files are stored in PostgreSQL’s main data directory (usually something like /var/lib/postgresql/data).
...
-- A new tablespace
CREATE TABLESPACE repositorio1 LOCATION '/your_path/repositorio_name1';
CREATE TABLESPACE repositorio2 LOCATION '/your_path/repositorio_name2';
...
CREATE TABLE tb_nome_tabela (
	...
	teste VARCHAR(40)
) TABLESPACE repositorio_name1;

/* REMOVER */
DROP TABLESPACE repositorio_name1;

/* MOVE */
-- Moving object from repositorio1 to repositorio2
ALTER DATABASE db_1 SET TABLESPACE repositorio_name2;
ALTER TABLE tb_nome_tabela SET TABLESPACE repositorio_name2;

/* RENAME */
ALTER TABLESPACE repositorio_name2 RENAME TO data3;
```
